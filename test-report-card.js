            ...
            it("The `addCourseRowToReportCard` function adds the correct course row HTML to the report card table for a given course and row number", () => {
                ...
                for (const semester of availableSemesters) {
                    ...
                    assert(
                        ...
                        `The 'addCourseRowToReportCard' function did not add the correct number course rows to the table div element, or the rows did not have the correct class names, or they were improperly formatted. Expected ${courses.length
                    ...
                    for (let rowNum = 0; rowNum < rowNodeList.length; rowNum++) {
                        ...
                        assert(
                            ...
                            `The ${rowNumberWord} row down had an incorrect class name representing its row number. Expected 'row-${rowNum + 1
                    ...
                    courses.forEach((course, rowNum) => {
                        ...
                        for (const col of columns) {
                            ...
                            assert(
                                ...
                                `The table column for ${label} in the ${semester} semester is either not created, uses an incorrect class, or has incorrect innerHTML content. Got ${columnElement?.innerHTML?.trim() ?? "null"
                        ...
                        assert(
                            ...
                            `The credits table column in the ${semester} semester does not include a child element with class="credits" or the innerHTML content for that element is incorrect. Got content ${creditsColumnInnerElement?.innerHTML ?? "null"
